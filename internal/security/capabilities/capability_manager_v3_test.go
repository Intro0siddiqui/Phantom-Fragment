...
func TestCapabilityConfig(t *testing.T) {
	...
	if config.MetricsInterval != 10*time.Second {
		t.Errorf("Expected MetricsInterval 10s, got %v", config.MetricsInterval)
	}
	...
	if config.Mode != "strict" {
		t.Errorf("Expected Mode 'strict', got '%s'", config.Mode)
	}
	...
	if len(config.AmbientCaps) != 1 {
		t.Errorf("Expected 1 ambient cap, got %d", len(config.AmbientCaps))
	}

	if len(config.BoundingSet) != 3 {
		t.Errorf("Expected 3 bounding set caps, got %d", len(config.BoundingSet))
	}
